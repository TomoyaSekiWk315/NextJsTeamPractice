        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-6 md:gap-6 lg:gap-12">
          <div className="md:col-span-2 space-y-4 md:space-y-6 text-center md:text-left">
            <Link href="/" className="inline-block group">
              <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 group-hover:text-[#005a64] transition-colors">
                MVP塗装
              </h2>
            </Link>
          ...
          <div className="md:col-span-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-6">
            {Object.entries(footerLinks).map(([category, { href, links }]) => (
              <div key={category} className="space-y-2 sm:space-y-3">
                <Link 
                  href={href}
                  className="inline-block text-xs sm:text-sm md:text-[11px] lg:text-base font-bold text-gray-900 hover:text-[#005a64] transition-colors"
                >
                  {category}
                </Link>
                {links.length > 0 && (
                  <ul className="space-y-1 sm:space-y-2">
                    {links.map((link) => (
                      <li key={link.title}>
                        <Link 
                          href={link.href} 
                          className="text-[10px] sm:text-xs text-gray-600 hover:text-[#005a64] transition-colors block"
                        >
                          {link.title}
                        </Link>
                      </li>
        ...
        <div className="mt-8 md:mt-16 pt-4 md:pt-8 border-t border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-4 md:gap-6 order-1 sm:order-2">
              <Link 
                href="/privacy-policy" 
                className="text-[10px] sm:text-xs text-gray-600 hover:text-[#005a64] transition-colors"
              >
                プライバシーポリシー
              </Link>
              <Link 
